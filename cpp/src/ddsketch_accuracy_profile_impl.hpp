...
#ifndef CHARACTERIZATION_DDSKETCH_ACCURACY_PROFILE_IMPL_HPP
#define CHARACTERIZATION_DDSKETCH_ACCURACY_PROFILE_IMPL_HPP
...
template<typename T>
void ddsketch_accuracy_profile<T>::run() {
  ...
  const std::vector<double> levels = {0.01, 0.05, 0.5, 0.95, 0.99};

  // Note: errors are multiplied by 100 below, so these are percentages.
  std::cout << "N";
  for (const double level: levels) std::cout << "\trel err (%) at " << level;
  std::cout << "\n";
  ...
  for (unsigned i = 0; i < num_steps; ++i) {
    ...
    for (auto& errors: quantile_errors) {
...
#endif //CHARACTERIZATION_DDSKETCH_ACCURACY_PROFILE_IMPL_HPP