...
output_dir = '../results/value_accuracy/plots'
os.makedirs(output_dir, exist_ok=True)

base = '../results/value_accuracy'
...
for rank_label, col in ranks.items():
    for dist_key, dist_title in dists.items():
        ...
        ax.set_ylabel('Quantile error', fontsize=14)
        ax.set_title(f'Quantile error, {dist_title}, level {rank_label}', fontsize=14)
        ax.legend(fontsize=10)
        ax.grid(True, which='both', linestyle='--', alpha=0.5)
        ax.tick_params(labelsize=12)
        fig.tight_layout()

        filename = f'{output_dir}/quantile_error_{dist_key}_level_{rank_label}.pdf'
